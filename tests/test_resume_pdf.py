...
import src.services.resume_fit_service as fit
...
def test_resume_pdf_page_count_detects_multipage_resume():
    long_bullet = (
        "Designed and implemented production-ready features across authentication, "
        "data processing, testing, deployment, and observability workflows."
    )
    snapshot = {
        "aggregated_skills": {
            "languages": ["Python", "TypeScript", "SQL", "Java"],
            "frameworks": ["FastAPI", "React", "Pytest", "Tailwind"],
            "technical_skills": [
                "API design",
                "Testing",
                "CI/CD",
            ...
            "writing_skills": ["Documentation", "Technical communication"],
        },
        "projects": [
            {
                ...
                "end_date": "2025-06-01",
                "contribution_bullets": [long_bullet] * 4,
            }
            for i in range(5)
        ],
    }
    ...
    user_profile = {
        ...
        "github": None,
        "location": "Kelowna, BC",
        "profile_text": "Experienced builder with strong interest in practical software systems.",
    }
    education_entries = [
        {
            "entry_id": 1,
    ...
    experience_entries = [
        {
            ...
            "company": "Example Co",
            "date_text": "May 2025 - Aug 2025",
            "description": "Built and maintained internal platforms used by multiple teams.",
        }
    ]

    page_count = exp.get_resume_record_pdf_page_count(
        username="john",
        record=record,
        user_profile=user_profile,
        education_entries=education_entries,
...
def test_resume_fit_status_warns_only_after_manual_project_edits(monkeypatch):
    monkeypatch.setattr(fit, "get_resume_record_pdf_page_count", lambda **kwargs: 2)

    autogenerated_record = {
        "resume_json": json.dumps(
            {
                "projects": [
                    {
                        "project_name": "Project Alpha",
                        "contribution_bullets": ["Built API", "Wrote tests"],
                    }
                ]
            }
        ),
        ...
        "resume_json": json.dumps(
            {
                "projects": [
                    {
                        "project_name": "Project Alpha",
                        "resume_contributions_override": ["Custom bullet 1", "Custom bullet 2"],
                    }
                ]
            }
        ),
    ...
    blocked = fit.build_resume_fit_status(username="john", record=autogenerated_record)
    warned = fit.build_resume_fit_status(username="john", record=manual_edit_record)

    assert blocked["overflow_mode"] == "block"
    assert blocked["has_manual_project_edits"] is False
    assert warned["overflow_mode"] == "warn"
    assert warned["has_manual_project_edits"] is True

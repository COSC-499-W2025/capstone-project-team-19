...
def fetch_one(conn, table, **filters):
    query = f"SELECT * FROM {table} WHERE " + " AND ".join(f"{k}='{v}'" for k,v in filters.items())
...
def test_insert_summary(conn):
    insert_summary(conn, user_id=1, project_name="proj1", drive_file_id="file123",
                   user_revision_count=3, total_word_count=100, total_revision_count=5)
    row = fetch_one(conn, "text_contribution_summary", project_name="proj1")
    assert row[1:] == (1, "proj1", "file123", 3, 100, 5)

...
def insert_version_summary(
    ...
    conn.execute(
        ...
        (
            ...
            lines_deleted,
            total_words,
            json.dumps(sorted(languages) if isinstance(languages, set) else languages) if languages else None,
            json.dumps(sorted(frameworks) if isinstance(frameworks, set) else frameworks) if frameworks else None,
            avg_complexity,
            total_files,
        ),
    )

...
from src.utils.image_utils import validate_image_path, copy_to_images_dir
...
def manage_project_thumbnails(conn, user_id: int, username: str) -> None:
    ...
    while True:
        ...
        if action == "1":
            if existing:
                print(f"\nThumbnail already exists for '{project_name}'. Use Edit instead.\n")
                continue
            raw_path = input("Enter image path: ").strip()
            try:
                src = validate_image_path(raw_path)
                dst = copy_to_images_dir(src, images_dir, user_id, project_name)

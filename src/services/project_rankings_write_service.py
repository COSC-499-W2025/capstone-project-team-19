...
def clear_project_rank(conn: Connection, user_id: int, project_name: str) -> None:
    with _transaction_scope(conn):
        clear_project_rank_query(conn, user_id, project_name)

def clear_all_rankings(conn: Connection, user_id: int) -> None:
    with _transaction_scope(conn):
        clear_all_rankings_query(conn, user_id)

def bulk_set_rankings(conn: Connection, user_id: int, rankings: List[Tuple[str, int]]) -> None:
    with _transaction_scope(conn):
...
def set_project_rank(
    ...
    with _transaction_scope(conn):
        ...
        if current_rank is None:
            ...
            shift_project_ranks_for_move_up(conn, user_id, manual_rank, current_rank, project_name)
        else:
            shift_project_ranks_for_move_down(conn, user_id, current_rank, manual_rank, project_name)

        upsert_project_rank(conn, user_id, project_name, manual_rank)
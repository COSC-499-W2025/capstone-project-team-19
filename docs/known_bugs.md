...
- **ZIP upload change before “Next” may trigger incorrect duplicate detection:** If a user uploads a ZIP file and then replaces or modifies it before clicking **Next**, the system may incorrectly flag it as a duplicate or already analyzed project.
- **Single-file text collaboration stored incorrectly (null instead of array):** When a text collaboration contains only one file, it may be stored as `null` instead of an empty array, causing errors in functions that expect an array.
- **Framework data not displayed in resume despite backend support:** Framework information is captured and available in the backend but is not currently rendered in the resume output.